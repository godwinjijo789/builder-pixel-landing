...
function dhash(img: HTMLCanvasElement): bigint {
  ...
  for (let y=0;y<h;y++){
    for (let x=0;x<w-1;x++){
      const i1 = (y*w + x)*4; const i2 = (y*w + x + 1)*4;
...
function IPCamera() {
  ...
  const students: any[] = useMemo(()=> JSON.parse(localStorage.getItem(`students:${profile?.schoolId||'SCHOOL'}`) || localStorage.getItem('students') || '[]'), [profile?.schoolId]);
  const hashesRef = useRef<{id:string; className:string; name:string; hash:bigint}[]>([]);
  const canvasRef = useRef<HTMLCanvasElement | null>(null);
  const detector = ("FaceDetector" in window) ? new (window as any).FaceDetector({ fastMode: true }) : null;

  useEffect(() => () => { stop(); }, []);

  const buildHashes = async () => {
    const list: {id:string; className:string; name:string; hash:bigint}[] = [];
    for (const s of students) {
      if (!s.faceImage) continue;
      const img = new Image();
      await new Promise<void>((r)=>{ img.onload=()=>r(); img.src=s.faceImage; });
      const c = document.createElement('canvas'); c.width = img.width; c.height = img.height; c.getContext('2d')!.drawImage(img,0,0);
      const h = dhash(c);
      list.push({ id: s.roll || s.id, className: s.className || 'Class 7', name: s.name || 'Student', hash: h });
    }
  ...
  const tick = async () => {
    ...
    try {
      ...
      for (const f of faces) {
        ...
        const faceCanvas = document.createElement('canvas'); faceCanvas.width = width; faceCanvas.height = height;
        faceCanvas.getContext('2d')!.drawImage(canvas, x, y, width, height, 0, 0, width, height);
        ...
        if (best && best.dist <= 10) {
          ...
          if (!rec[best.cand.id]) {
            ...
            setLog((l)=>[`${best.cand.name} (${best.cand.id}) marked Present`, ...l].slice(0,20));
            try { await fetch('/api/attendance', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ event: { doId, schoolId, date, className: best.cand.className, studentId: best.cand.id, status: 'P' } }) }); } catch {}
          }
        }
      }
    } catch {}
  ...
  const uploadAbsentees = async () => {
    ...
    for (const cls of Object.keys(grouped)) {
      ...
      for (const s of grouped[cls]) {
        if (!rec[s.roll]) {
          try { await fetch('/api/attendance', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ event: { doId, schoolId, date, className: cls, studentId: s.roll, status: 'A' } }) }); } catch {}
        }
      }
    }
        ...
        <div className="flex gap-2">
          {!started ? (
            ...
            <>
              ...
              <Button variant={detecting?"default":"outline"} onClick={()=>setDetecting((d)=>!d)} disabled={!detector}>{detecting?"Stop Detection":"Start Detection"}</Button>
              <Button variant="outline" onClick={uploadAbsentees}>Upload Absentees</Button>
            </>
          )}
        </div>
        {!detector && <div className="text-xs text-yellow-600">FaceDetector API not available in this browser. Detection will be disabled.</div>}
