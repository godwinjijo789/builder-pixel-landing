...
const CAMS = [
  ...
  { id: 3, name: "CCTV Camera 3", location: "Playground - School B", status: "online", last: "10 minutes ago", uptime: "12 days", img: "https://images.unsplash.com/photo-1565619624098-2f4b4f32ed7f9?q=80&w=800&auto=format&fit=crop" },
...
export default function SystemStatus() {
  ...
  const unresolvedMaint = useMemo(() => MAINT.filter((m) => !resolved.includes(m.id)), [resolved]);
          ...
          {CAMS.map((c) => (
            <Card key={c.id}>
              ...
              <CardContent>
                ...
                <div className={`mt-2 text-sm font-medium ${c.status === "online" ? "text-emerald-600" : "text-red-600"}`}>{c.status === "online" ? "Online" : "Offline"}</div>
                <div className="text-xs text-muted-foreground">Last Check-in: {c.last}</div>
                <div className="text-xs text-muted-foreground">Uptime: {c.uptime}</div>
              </CardContent>
              <CardFooter>
                <Button variant="outline" className="ml-auto" onClick={() => viewCam(c.id)}>View Details</Button>
              </CardFooter>
            </Card>
          ))}
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {unresolvedMaint.map((m) => (
            <Card key={m.id}>
              ...
              <CardFooter>
                <Button variant="destructive" onClick={() => markResolved(m.id)}>Resolve Issue</Button>
              </CardFooter>
            </Card>
          ))}
          {unresolvedMaint.length === 0 && (
            <Card>
              <CardHeader>
                <CardTitle className="text-base">No outstanding maintenance issues</CardTitle>
              </CardHeader>
              <CardContent>
                <div className="text-sm text-muted-foreground">All systems are healthy.</div>
              </CardContent>
            </Card>
          )}
        </div>
          ...
          <DialogContent className="sm:max-w-2xl">
            ...
            {cam && (
              <div className="grid gap-3">
                <img src={cam.img} alt={cam.name} className="w-full h-56 object-cover rounded-md border" />
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-sm">
                  <div><span className="text-muted-foreground">Name: </span>{cam.name}</div>
                  <div><span className="text-muted-foreground">Location: </span>{cam.location}</div>
                  <div><span className="text-muted-foreground">Status: </span><span className={cam.status === 'online' ? 'text-emerald-600' : 'text-red-600'}>{cam.status}</span></div>
                  <div><span className="text-muted-foreground">Uptime: </span>{cam.uptime}</div>
                  <div><span className="text-muted-foreground">Last check-in: </span>{cam.last}</div>
                </div>
                <div className="flex justify-end gap-2">
                  <Button variant="outline" onClick={closeCam}>Close</Button>
                  <Button onClick={() => { closeCam(); navigate('/cctv'); }}>Go to CCTV</Button>
                </div>
              </div>
            )}
          </DialogContent>
